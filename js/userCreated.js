...
const UserCreatedApp = (function(MainApp) {
  ...
  function startUserQuiz() {
    ...
    // クイズ画面を表示
    const quizScreen = document.getElementById('userQuizScreen');
    if (quizScreen) {
  ...
  function retryUserQuiz() {
    ...
    // クイズ開始前画面を表示する
    const quizStartScreen = document.getElementById('userQuizStartScreen');
    if (quizStartScreen) {
  ...
  function backToUserCreatedMenu() {
    MainApp.hideAllScreens();
    const menuScreen = document.getElementById('userQuizStartScreen'); // または別のメニュー画面
    if (menuScreen) {
